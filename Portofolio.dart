...
class InputNilaiPortofolio extends StatefulWidget {
  const InputNilaiPortofolio({Key? key, required User user}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _InputNilaiPortofolioState createState() => _InputNilaiPortofolioState();
}
...
class _InputNilaiPortofolioState extends State<InputNilaiPortofolio> {
  ...
  void generateStudentList(
    ...
    for (CardModel card in cards) {
      ...
      final DocumentSnapshot doc = await siswaCollection.doc(subjectId).get();
      if (doc.exists) {
        final data = doc.data();
        if (data != null && data is Map<String, dynamic>) {
          final tugasData = data['tugas'] as Map<String, dynamic>?;
          ...
          if (tugasData != null) {
            ...
            // Jika field 'tugas' adalah Null, set nilai default pada kartu
            for (int i = 0; i < 8; i++) {
              card.values[i] = 0;
              card.textEditingControllers[i].text = '';
            }
          ...
          // Print the fetched data for each card
          print('Card Title: ${card.title}');
          print('Values: ${card.values}');
          print('Text Editing Controllers: ${card.textEditingControllers}');
        }
      }
    }
  }
