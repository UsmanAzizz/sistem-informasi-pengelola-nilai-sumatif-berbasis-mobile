...
class InputNilaiPSAS extends StatefulWidget {
  const InputNilaiPSAS({Key? key, required User user}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _InputNilaiPSASState createState() => _InputNilaiPSASState();
}
...
class _InputNilaiPSASState extends State<InputNilaiPSAS> {
    ...
    for (CardModel card in cards) {
      ...
      if (doc.exists) {
        ...
        if (data != null && data is Map<String, dynamic>) {
          final psasData = data['psas'] as Map<String, dynamic>?;

          if (psasData != null) {
            for (int i = 0; i < 8; i++) {
              final String fieldName = '${i + 1}';
              final int fieldValue = psasData[fieldName] as int? ?? 0;
              setState(() {
            ...
            // Jika field 'psas' adalah Null, set nilai default pada card
            for (int i = 0; i < 8; i++) {
              setState(() {
                card.values[i] = 0;
                card.textEditingControllers[i].text = '';
  ...
  void saveValues() {
    ...
    Future.wait(cards.map((card) async {
      ...
      final DocumentSnapshot doc =
          await siswaCollection.doc(selectedSubject).get();
      if (doc.exists) {
        await doc.reference.update({'psas': nilaiSiswa});
      } else {
        // Jika dokumen siswa belum ada, buat dokumen secara otomatis
        await siswaCollection.doc(selectedSubject).set({'psas': nilaiSiswa});
      }
