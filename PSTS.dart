...
class InputNilaiPSTS extends StatefulWidget {
  const InputNilaiPSTS({Key? key, required User user}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _InputNilaiPSTSState createState() => _InputNilaiPSTSState();
}
...
class _InputNilaiPSTSState extends State<InputNilaiPSTS> {
  ...
  Future<void> fetchStudentData() async {
    ...
    for (CardModel card in cards) {
      if (classData[selectedSubject] != null &&
        ...
        final pstsData = classData[selectedSubject][card.title]['psts'] as Map?;
        if (pstsData != null) {
          for (int i = 0; i < 8; i++) {
            final String fieldName = '${i + 1}';
            final int fieldValue = pstsData[fieldName] as int? ?? 0;
  ...
  void saveValues() async {
    ...
    final DocumentReference<Map<String, dynamic>> kelasDocument =
        FirebaseFirestore.instance.collection('nilaimapel').doc(selectedClass);
    ...
    for (CardModel card in cards) {
      final Map<String, dynamic> pstsData = {};
      for (int i = 0; i < 8; i++) {
        final String fieldName = '${i + 1}';
        pstsData[fieldName] = card.values[i];
      }

      // Create a new 'psts' field for each student
      classData[selectedSubject] ??= {};
      classData[selectedSubject][card.title] ??= {};
      classData[selectedSubject][card.title]['psts'] = pstsData;
    }
            ...
            child: isLoading
                ? Center(
                    child: Column(
                      ...
                      children: const [
                        SpinKitWave(
                          color: Colors.green, // Ubah warna Spinkit
                          size: 50.0, // Ubah ukuran Spinkit
                        ),
                        SizedBox(height: 16),
                        Text(
                          'Memuat data...',
                ...
                : isSaving
                    ? Center(
                        child: Column(
                          ...
                          children: const [
                            SpinKitFadingCube(
                              color: Colors.pink, // Ubah warna Spinkit
                              size: 45.0, // Ubah ukuran Spinkit
                            ),
                            SizedBox(height: 36),
                            Text(
                              'menyimpan nilai..',
