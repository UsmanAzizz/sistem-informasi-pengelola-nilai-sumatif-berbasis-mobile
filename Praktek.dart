...
class InputNilaiPraktek extends StatefulWidget {
  const InputNilaiPraktek({Key? key, required User user}) : super(key: key);

  @override
  // ignore: library_private_types_in_public_api
  _InputNilaiPraktekState createState() => _InputNilaiPraktekState();
}
...
class _InputNilaiPraktekState extends State<InputNilaiPraktek> {
    ...
    for (CardModel card in cards) {
      ...
      if (doc.exists) {
        ...
        if (data != null && data is Map<String, dynamic>) {
          final praktekData = data['praktek'] as Map<String, dynamic>?;

          if (praktekData != null) {
            for (int i = 0; i < 8; i++) {
              final String fieldName = '${i + 1}';
              final int fieldValue = praktekData[fieldName] as int? ?? 0;
              setState(() {
            ...
            // Jika field 'praktek' adalah Null, set nilai default pada card
            for (int i = 0; i < 8; i++) {
              setState(() {
                card.values[i] = 0;
                card.textEditingControllers[i].text = '';
  ...
  void saveValues() {
    ...
    Future.wait(cards.map((card) async {
      ...
      final DocumentSnapshot doc =
          await siswaCollection.doc(selectedSubject).get();
      if (doc.exists) {
        await doc.reference.update({'praktek': nilaiSiswa});
      } else {
        // Jika dokumen siswa belum ada, buat dokumen secara otomatis
        await siswaCollection.doc(selectedSubject).set({'praktek': nilaiSiswa});
      }
